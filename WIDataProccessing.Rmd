...
starbucks_WI <- starbucks %>%
  filter(Country== 'US', State=='WI')%>% 
...
addMarkers(data = starbucks_WI, lat = ~ Latitude, lng = ~ Longitude, popup = starbucks_WI$Name) %>%
  addPopups(-89.360922, 43.226851, 'the <b> spot</b>')
...
Excel
```{r}
starbucks_WI_R_C_P <- starbucks_WI %>%
...
```{r}
ggplot(data = starbucks_WI_R_C_P) +
...
ggplot(starbucks_WI_R_C_P, aes(P)) + 
  geom_histogram()
...
ggplot(starbucks_WI_R_C_P, aes(x=C, y=`Ownership Type`)) + geom_point()
